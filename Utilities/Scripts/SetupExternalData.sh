...
say_store() {
  if test -f "$config_store"; then
    echo 'A default for VTKSTREAMING_DATA_STORE is configured as:' &&
    echo &&
    sed 's/^/  /' < "$config_store" &&
    echo
  else
    echo 'No default for VTKSTREAMING_DATA_STORE is configured.' &&
    echo
  fi
}
...
select a default for VTKSTREAMING_DATA_STORE [n/h/s]: ' ans &&
    case "$ans" in
      n|N|no) ans='no' ;;
...
store_sibling="$(native_path "${PWD%/*}/VTKStreamingExternalData")" &&
echo 'VTKStreaming may download data objects into a local "store" using
the CMake ExternalData module.  The store is content-addressed
and can be shared across multiple projects and build trees.
VTKStreaming build trees have a VTKSTREAMING_DATA_STORE CMake cache entry to set
...
say_exclude() {
  if test -f "$config_exclude"; then
    echo 'A default for VTKSTREAMING_DATA_EXCLUDE_FROM_ALL is configured as:' &&
    echo &&
    sed 's/^/  /' < "$config_exclude" &&
    echo
  else
    echo 'No default for VTKSTREAMING_DATA_EXCLUDE_FROM_ALL is configured.' &&
    echo
  fi
}
...
ask_exclude() {
 ...
 n,no,off    = Include VTKStreamingData target in default build
 y,yes,on    = Exclude VTKStreamingData target from default build

select a default for VTKSTREAMING_DATA_EXCLUDE_FROM_ALL [d/n/y]: ' ans &&
    case "$ans" in
      d|D|delete) ans='D' ;;
      n|N|no|off) ans='OFF' ;;
      y|Y|yes|on) ans='ON' ;;
...
echo 'VTKStreaming defines a "VTKStreamingData" build target to download data
objects at build time to make them available for running tests.
VTKStreaming build trees have a VTKSTREAMING_DATA_EXCLUDE_FROM_ALL CMake
cache option to exclude the "VTKStreamingData" target from being built
