"""Exercise the NVENC H.264 encoder with IYUV, NV12 and RGBA32 raw frame inputs.

moving color bars are generated in memory and uploaded straight to the
vtkOpenGLVideoFrame. There is no NVDEC decoder module, so instead of an image
round-trip the test checks that the emitted bitstream is a plausible H.264
Annex B stream and not all zeros.
"""
...
def test_nv_encode_simple(format_name):
